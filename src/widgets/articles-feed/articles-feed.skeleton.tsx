...
export function ArticlesFeedSkeleton() {
  return new Array(5).fill(0).map((_, idx) => (
    <div key={`static-${String(idx)}`} className="article-preview">
  ...
  return (
    <Stack direction="column" spacing={2}>
      {new Array(4).fill(0).map((_, idx, array) => (
        <Skeleton key={`static-${String(idx)}`} variant="text" width={array.length - 1 === idx ? '60%' : '100%'} />
      ))}
    </Stack>
  );
}
  ...
  return (
    <Stack spacing={4}>
      {new Array(4).fill(0).map((_, idx) => (
        <Skeleton key={`static-${String(idx)}`} variant="rounded" width={45} />
      ))}
    </Stack>
  );
}
