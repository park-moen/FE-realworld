...
import type { PrimaryLoaderArgs, SecondaryLoaderArgs } from './filter-article.types';

export function PrimaryFilter() {
  const session = useSelector(selectSession);
  const {
    context: {
      filterQuery: { source, tag },
    },
  } = useLoaderData<PrimaryLoaderArgs>();
  const [, setSearchParams] = useSearchParams();

  const tabValue = tag ? 'tag' : source;

  const handleTabClick = (sourceParam: string) => {
    setSearchParams(new URLSearchParams({ source: sourceParam, page: '1' }));
  };
...
export function SecondaryFilter() {
  const { params, context } = useLoaderData<SecondaryLoaderArgs>();
  const [, setSearchParams] = useSearchParams();
  ...
  const handleTabClick = (value: string) => {
    const newParams = new URLSearchParams({
      ...filterQuery,
      page: String(filterQuery.page),
      limit: String(filterQuery.limit),
    });
