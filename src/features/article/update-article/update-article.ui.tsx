...
import { UpdateArticleSchema } from './update-article.contracts';
...
function BaseUpdateArticleForm({ slug }: UpdateArticleFormProps) {
  ...
  const { mutate, isPending, isError, error } = useUpdateArticleMutation({
    mutationKey: [slug],
    onSuccess: (updateArticle) => {
      navigate(pathKey.article.bySlug(updateArticle.slug), { replace: true });
    },
  });
