...
export function useUnfollowProfileMutation(
  ...
  return useMutation({
          ...
          const updatedArticles = Object.fromEntries(
            Object.entries(articles).map(([slug, rawArticle]) => [
              ...
              rawArticle.author.username === username
                ? { ...rawArticle, author: { ...rawArticle.author, following: false } }
                : rawArticle,
            ]),
          );
    ...
    onError: async (error, username, onMutationResult, context) => {
      const articleQueryKey = ARTICLES_ROOT_QUERY_KEY;
      const profileQueryKey = profileQueryOptions(username).queryKey;
      const { previousArticles, previousProfile } = onMutationResult || {};
      ...
      await onError?.(error, username, onMutationResult, context);
    },
